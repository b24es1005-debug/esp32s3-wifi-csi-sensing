...
class StreamingPreprocessor:
    ...
    def push(self, raw_amplitudes: list) -> Optional[StreamResult]:
        ...
        """
        arr = np.array(raw_amplitudes, dtype=np.float32)
        ...
        if len(self._raw) >= 5:
            recent_list = list(self._raw)[-min(15, len(self._raw)):]

# Ensure all arrays have same length
            min_len = min(len(arr) for arr in recent_list)

            recent = np.stack(
                [arr[:min_len] for arr in recent_list],
                axis=0
            )
...
# Force equal shape before stacking
        min_len = min(len(arr) for arr in buffer_list)

        window = np.stack(
            [arr[:min_len] for arr in buffer_list],
            axis=0
        )
